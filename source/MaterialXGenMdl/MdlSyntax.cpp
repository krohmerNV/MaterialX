...
const TypeDesc* MDL_SHEEN_MODE = TypeDesc::registerType("mode", TypeDesc::BASETYPE_NONE, TypeDesc::SEMANTIC_ENUM, 0);
...
string MdlSyntax::replaceSourceCodeMarkers(const string& nodeName, const string& soureCode, std::function<string(const string&)> lambda, bool removeParam) const
{
    ...
    while (i != string::npos)
    {
        ...
        const string value = lambda(marker);
        if (removeParam && value.empty())
        {
            code.pop_back(); // drop the parameter and rely on default values in the MDL implementation
        }
        else
        {
